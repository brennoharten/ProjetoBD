...
async function iniciarBanco(
  ...
  for (i = 0; i < bucketAmount; i++) buckets.push(new Bucket(bucketSize, i));

  for (i = 0; i < pages.length; i++) {
    pages[i].content.forEach((tuple) => {
      if (tuple) buckets[hash(tuple, bucketAmount)].addTuple(tuple, i);
    });
  }
...
function checkPageQuantity(e) {
  document.getElementById("pageAmount").value = Math.round(
    table.length / e.value
  );
}
...
function generateStatistics() {
  ...
  buckets.forEach((bucket) => {
    if (bucket.isFull()) fullBuckets++;
    if (bucket.content.length == 0) emptyBuckets++;
    else usedBuckets++;
  });
  document.getElementById("container2").style.display = "block";
